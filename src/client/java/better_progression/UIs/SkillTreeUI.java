...
public class SkillTreeUI extends Screen {
    ...
    protected void init() {
        ...
        SkillTree.REGISTRY.values().forEach(tree -> tree.getNodes().values().forEach(node -> {
            allGuiNodes.add(node);
            ImageButton button = new ImageButton(0, 0, 20, 20, new WidgetSprites(node.getSkill().iconId()), button1 -> {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
        ...
        allGuiNodes.forEach(node -> {
            ...
            if (btn != null) {
                ...
                guiGraphics.blitSprite(RenderPipelines.GUI_TEXTURED, Skills.BUTTON_BACKGROUND_OBTAINED,
                        actualX - 2, actualY - 2, 22, 22);
            }
        });
